export class Vector3 { constructor(x=0,y=0,z=0){this.x=x;this.y=y;this.z=z;}
 ...
 clone(){return new Vector3(this.x,this.y,this.z);} add(v){this.x+=v.x;this.y+=v.y;this.z+=v.z;return this;} addScaledVector(v,s){this.x+=v.x*s;this.y+=v.y*s;this.z+=v.z*s;return this;} multiplyScalar(s){this.x*=s;this.y*=s;this.z*=s;return this;} }
...
export class Object3D { constructor(){this.position=new Vector3();this.rotation=new Euler();this.scale=new Vector3(1,1,1);this.children=[];this.parent=null;this.visible=true;this.quaternion=new Quaternion();this.matrixWorld={decompose(p,q,s){p.set(1,2,3);}};}
 ...
 updateWorldMatrix(){} getWorldPosition(v){return v.copy(this.position);} lookAt(){} clone(){const c=new this.constructor(this.geometry,this.material);c.position.copy(this.position);c.rotation.copy(this.rotation);c.scale.copy(this.scale);return c;} }
